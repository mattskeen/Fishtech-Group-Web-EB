...
* License.
*/
class CP_HubSpot_BaseClient
{
    // CP_HubSpot_BaseClient class to be extended by specific hapi clients
     ...
     * clients that extend CP_HubSpot_BaseClient should set $API_PATH to the
     * base path for the API (e.g.: the leads api sets the value to
     * 'leads')
     *
     * @throws CP_HubSpot_Exception
     */
    protected function get_api()
    {
        if ( $this->isBlank($this->API_PATH) )
            throw new CP_HubSpot_Exception('API_PATH must be defined');
        else
     ...
     * clients that extend CP_HubSpot_BaseClient should set $API_VERSION to the
    ...
    protected function get_api_version()
    {
        if ( $this->isBlank($this->API_VERSION) )
            throw new CP_HubSpot_Exception('API_VERSION must be defined');
        else
    ...
    protected function execute_get_request($url)
    {
        ...
        if ( $errno > 0 )
            throw new CP_HubSpot_Exception('cURL error: ' . $error);
        else
            return $output;
    }
    ...
    protected function execute_put_request($url, $body)
    {
        ...
        if ( $apierr > 0 )
            throw new CP_HubSpot_Exception('cURL error: ' . $errmsg);
        else
            return $result;
    }
